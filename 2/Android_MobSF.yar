rule appguard : packer
{
  ...
  condition:
    true and ($stub and $encrypted_dex)
}
...
rule appguard_b : packer
{
  ...
  condition:
    true and (2 of ($a*) and 1 of ($b*) and 1 of ($c*))
}
...
private rule secneo_base
{
  ...
  condition:
    true and any of ($encrypted_dex, $encryptlib2, $encryptlib1)
}
...
rule dexprotector : packer
{
  ...
  condition:
    true and 1 of ($encrptlib_*) and 1 of ($asset*)
}
...
rule dexprotector_b : packer
{
  ...
  condition:
    true and 2 of ($encrptlib_*) and $encrptcustom and
...
rule bangcle : packer
{
  ...
  condition:
    true and any of ($main_lib, $second_lib, $container, $encrypted_jar, $encrypted_jar2)
}
...
rule unicom_loader : packer
{
  ...
  condition:
    true and ($unicom_lib and ($decrypt_lib or $classes_jar))
}
...
rule nqshield : packer
{
  ...
  condition:
    true and any of ($lib, $lib_sec1, $lib_sec2)
}
...
rule tencent : packer
{
  ...
  condition:
    true and ($decryptor_lib or $zip_lib or $mix_dex)
}
...
rule ijiami : packer
{
  ...
  condition:
    true and ($old_dat or $new_ajm or $ijm_lib)
}
...
rule medusah_appsolid : packer
{
  ...
  condition:
    true and $encrypted_dex and not medusah
}
...
rule kony : packer
{
  ...
  condition:
    true and $lib and $decrypt_keys and $encrypted_js
}
...
rule yidun : packer
{
  ...
  condition:
    true and (#lib > 1) or ($anti_trick and $entry_point and $jni_func)
}
...
rule appsealing_a : packer
{
  ...
  condition:
    true and $lib and $a1 and 1 of ($b*)
}
